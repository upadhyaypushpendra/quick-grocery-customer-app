...
export default function RegisterPage() {
  ...
  const handleVerifyOtp = (e: React.FormEvent) => {
    ...
    verifyOtp.mutate(
      ...
      {
        onSuccess: () => {
          toast.success('Welcome to QuickGrocery!');
          navigate('/');
        },
    ...
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-brand-50 to-brand-100 py-12">
      <div className="w-full max-w-md bg-gray-800 rounded-lg shadow-lg p-8">
        ...
        {step === 'details' ? (
          ...
          <form onSubmit={handleVerifyOtp} className="space-y-5">
            ...
            <button
              ...
              className="w-full bg-brand-600 text-white py-3 rounded-lg font-bold hover:bg-brand-700 disabled:bg-gray-600 disabled:cursor-not-allowed transition"
            >
              {verifyOtp.isPending ? 'Verifying...' : 'Verify & Sign Up'}
            </button>
        ...
        <div className="flex items-center gap-3 my-6">
          ...
          <span className="text-brand-600 text-sm">Already have an account?</span>
          <div className="flex-1 h-px bg-brand-200"></div>
        </div>
